...
export async function NotificationsPreview({ lang = "al" }: { lang?: Lang }) {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="mt-1 font-serif text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
            {lang === "en" ? "Latest News" : "Njoftimet e fundit"}
          </h2>
        </div>
        <Link
        ...
        >
          ...
          {lang === "en" ? "View all" : "Shiko të gjitha"}
        </Link>
      </div>

      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {notifications.map((n) => (
          <Link key={n.id} href={`${prefix}/notifications/${n.id}`} className="block hover:opacity-90 transition-opacity">
            <article className="rounded-2xl border border-border bg-card overflow-hidden flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-1">
                <p className="text-xs font-medium text-primary mb-2">
                  {new Date(n.created_at).toLocaleDateString(lang === "en" ? 'en-US' : 'sq-AL', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })}
